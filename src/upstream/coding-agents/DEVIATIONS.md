...
- `src/core/survey.ts` restructures the survey lease-key derivation so a standalone single-line `codeql[js/insufficient-password-hash]` suppression sits directly above the single-statement `createHash("sha256").update(leaseKeyInput)` sink (CodeQL only honors a standalone comment on the line immediately above the alert line; a comment inside the method chain is not honored). CodeQL flags the fast SHA-256 over `apiUrl + apiToken + bankId` as a weak password hash, but this digest is a deterministic lease lookup key that keeps credentials out of scratch paths — it is neither password storage nor verification, and no verification oracle exists for it, so a fast hash is correct and the alert is a false positive.
