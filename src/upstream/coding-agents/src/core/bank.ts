import { managedBank } from "@memory-router/coding-agents/runtime";
/**
...
export function isOptedIn(config: BankConfig, directory: string): boolean {
  try {
    managedBank((config as BankConfig & { routerHarness?: string }).routerHarness, directory);
    return true;
  } catch { return false; }
}
...
export function deriveBankId(
  ...
  return managedBank(harness, directory);
}
