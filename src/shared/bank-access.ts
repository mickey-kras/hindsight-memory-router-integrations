...
export function visibleBanks(access: BankAccess): string[] {
  const banks = [
    ...new Set([
      ...(access.writeBank ? [access.writeBank] : []),
      ...access.additionalReadBanks,
    ]),
  ];
  if (
    banks.some(
      (bank) => !BANK_ID_PATTERN.test(bank) || bank === "." || bank === "..",
    )
  ) {
...
export function requireBank(
  ...
  if (
    !visibleBanks(access).includes(bank) ||
    (operation === "write" && access.writeBank !== bank)
  ) {
    throw new AccessDeniedError();
  }
}
...
export function classifyOperation(
  ...
  if (method === "POST" && ["/memories/recall", "/reflect"].includes(suffix))
    return "read";
  if (
    method === "GET" &&
