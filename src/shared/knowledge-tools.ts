...
export function routedKnowledgeTools(transport: RouterTransport) {
  ...
  return definitions.map((tool) => ({
    ...
    async execute(params: Record<string, unknown>) {
      ...
      let body: unknown;
      switch (tool.name) {
        case "agent_knowledge_list_pages":
          suffix += "?detail=metadata";
          break;
        case "agent_knowledge_get_page":
          suffix += `/${page}`;
          break;
        ...
        case "agent_knowledge_update_page":
          ...
          break;
        case "agent_knowledge_delete_page":
          suffix += `/${page}`;
          method = "DELETE";
          break;
        case "agent_knowledge_ingest":
          ...
          body = {
            async: true,
            items: [{ content: params.content, document_id: String(params.title).toLowerCase().replaceAll(" ", "-") }],
          };
          break;
        default:
