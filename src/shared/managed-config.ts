...
export function loadManagedConfig(
  ...
  try {
    if (!configPath || !isAbsolute(configPath) || !principalId) throw new AccessDeniedError();
    const config = JSON.parse(readFileSync(configPath, "utf8")) as ManagedRouterConfig;
